# Daily Automation — Agent Operating Guide

This repo is a public-safe reference implementation for a Hermes Agent / Claude Code daily-operations system.

It combines Markdown skills, local scripts, Notion, Google Calendar/Tasks, and an Obsidian-style vault into a trust-gated Chief-of-Staff workflow.
...
.claude/agents/      Optional subagent prompts
config/              Sanitized source and stream configuration
scripts/             Helper scripts and validation gates
templates/           Empty/sanitized Notion templates
tests/               Unit tests for pure helper logic
state/               Local runtime mode/defaults only; temp data ignored
logs/                Local-only runtime logs
vault/               Local-only Obsidian-style notes
.context/            Local-only staged/applied write payloads
```
