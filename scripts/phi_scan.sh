#!/usr/bin/env bash
# scripts/phi_scan.sh — input gate for /capture-meeting. Scans stdin for sensitive info markers.
#
# Usage:
#   echo "$NOTES" | scripts/phi_scan.sh
#   if scripts/phi_scan.sh < notes.txt; then echo "clean"; else echo "sensitive info detected"; fi
#
# Exit codes:
#   0  — no sensitive info markers detected. Safe to proceed.
#   1  — sensitive info markers detected. Caller MUST refuse and prompt user to clean the input.
#   2  — read error.
#
...
#   - This is NOT a HSalesA-compliance tool. It's a "stop the dumbest leakage" gate.
#   - the operator's actual workflow rarely involves client identifiers; the meetings
#     are operational (contacts, urgent cares, panels). sensitive info showing up is almost
#     always an accident.
#
...
if hits:
    print("sensitive info markers detected:", file=sys.stderr)
