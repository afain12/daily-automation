...
CLI:
    capture_meeting_parse.py (--notes-file PATH | --stdin) --date YYYY-MM-DD
                             --meeting-key STR
                             [--business-tag {nestmate,dock_pro,ipa,lab,other}]
...
VALID_BUSINESS_TAGS = ("nestmate", "dock_pro", "ipa", "lab", "other")
...
STRONG_INDICATORS = {
    ...
    "spoke with Dr", "credentialing", "NPI",
}
...
def _infer_secondary_hint(line: str, primary: str, cbk: dict[str, list[str]]) -> str | None:
    ...
    pattern noted in sync-sweep-tasks.md (Dr Tim ENG → lab even in nestmate section).
    """
    low = line.lower()
...
def _resolve_owner(line: str, attendees: list[str]) -> tuple[str, str]:
    """Return (owner, owner_resolved_from). Default to Aaron when not found."""
...
def _resolve_due(line: str, anchor: date) -> tuple[str | None, str | None]:
    ...
    # tomorrow + Aaron's recurring spellings (tomm/tommrow/tommorw/…)
...
#
# Aaron writes imperative scheduling directives into the daily note as checkbox
...
# Aaron's tomorrow spellings (reused by _resolve_due above and the classifier).
...
def extract_annotations(notes: str, anchor_date: date) -> list[dict[str, Any]]:
    ...
    for i, raw in enumerate(notes.splitlines(), start=1):
        ...
        if not stripped:
            continue
        # Skip briefing structure (not Aaron's hand-typed directives):
...
def _self_test() -> int:
    ...
    if cfg is not None:
        items = parse_notes(
            ...
            business_tag="nestmate",
            attendees=["Aaron", "Abos"],
            config=cfg,
        )
        if len(items) != 4:
        ...
        expected = {
            ...
            2: "task",           # "Tell Abos to follow up..."
            3: "contact",        # "Spoke with Dr Rookwood about credentialing..."
    ...
    anno_fixture = (
        ...
        "- [ ] Patrick credentialing ceate 11am task on calender\n"
        "- [ ] 15:00 Union NJ — Telcor close set for 4pm tomm\n"
        "- [ ] Dr Suleman postpone until next wednsay 12pm creasae calender event\n"
