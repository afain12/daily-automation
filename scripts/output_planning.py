...
renderers needed to prove the roundtrip. OutputScorecard, real_vs_tracked_delta
and earliest_start_hint are intentionally OUT of scope (see TASKS.md cut-list).
`CalendarBlockProposal` and the scheduling engine now live in their own pure
module `scripts/calendar_planning.py` (it imports `DailyOutput`/`SourceRef` from
here) — deliberately kept OUT of this parser-critical renderer.
"""
