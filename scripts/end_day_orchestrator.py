...
def merge_previews(ctx: RunContext, previews: list[SkillPreview]) -> EndDayRunPlan:
    ...
    owner_rank = {"end_day": 0, "note_harvest": 1, "sync_sweep": 2}
...
def build_preview(ctx: RunContext) -> EndDayRunPlan:
    workers = [preview_end_day_core, preview_sync_sweep, preview_note_harvest]
    previews: list[SkillPreview] = []
    ...
    previews.sort(key=lambda p: {"end_day": 0, "note_harvest": 1, "sync_sweep": 2}.get(p.skill, 9))
