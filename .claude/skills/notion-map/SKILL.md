...
with open("'"$SCORED"'") as f:
    data = json.load(f)
out = []
for role in ["master_tasks", "provider_crm", "activity_log", "meeting_notes"]:
...
- **Provider CRM**: stage/last_contact/next_step/workspace fields.
...
notion_databases:
  - name: "Master Tasks"
    ...
    # ["Lincoln Lab", "United IPA", "Nestmate", "Dock Pro", "Other"]
...
Roles mapped: {{N_ROLES}} / 4
  ...
  Provider CRM  → "Provider CRM"   ( 4 fields)
...
Run against Aaron's workspace. The proposal at `state/notion-mapping-{TODAY}.yaml`
...
- Provider CRM: stage_field, last_contact_field, next_step_field, workspace_field
