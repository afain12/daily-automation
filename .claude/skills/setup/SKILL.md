...
1. Read it. If it parses as YAML and contains a `streams:` key with ≥1 entry,
   treat it as the **incumbent config** (this is the case for the operator's repo —
...
- **Email** — Email triage and follow-up
...
Do **not** ask for: Notion API token, Google OAuth status, Email credentials,
...
notion_role_mapping: {} # master_tasks_data_source_id, contact_crm_data_source_id, etc.
...
block the the operator-version uses (paraphrased — don't hardcode "Operations" etc.):
