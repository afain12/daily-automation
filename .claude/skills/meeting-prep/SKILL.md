...
name: meeting-prep
description: >-
  Pre-meeting context staging. Pulls the next calendar event (or one the operator
  picks), identifies attendees, queries Contact CRM + Activity Log + recent
...
You are staging context for a meeting the operator is about to walk into. The output
...
the operator can pass the meeting details manually if gws is unavailable.
...
> - **Specific time** — the operator types a time like "11am" or "1:30pm" and we
>   find the matching event in today's calendar.
> - **By attendee** — the operator types a name; search today + tomorrow's calendar
>   for any event whose attendees include that name.
> - **Manual** — the operator types meeting title + attendees; skip calendar pull.
...
## Step 2: Resolve Attendees to Notion Contact Records

For each attendee (skip the operator himself), search the **Contact CRM**
...
business. If still ambiguous, flag for the operator at the dossier render step.
...
distinct from per-person history (e.g. for "Vendor A AI billing call", the
...
### {Attendee 2 Name}
- _Not in Contact CRM._ Vault hits:
  - {path} — "{snippet}"
...
Useful when the operator is between meetings and wants 30 seconds, not 3 minutes.
...
- **Attendee not in CRM:** still surface vault hits + Activity Log matches.
  the operator decides whether to create a CRM stub after the meeting via /capture-meeting.
...
- **Recurring 1:1s** — the per-attendee history is repetitive across runs.
  After 3-4 meetings with the same person, the operator knows their context.
  Use `--brief` mode instead.
- **External meetings (vendor demos)** — the attendees are unlikely to be in
  Contact CRM. Output will be sparse. Manually entering "what I want to
  cover" is faster.
- **Internal team meetings** — Contact CRM is client/contact-shaped, not
  team-shaped. Activity Log has internal entries but they're sparse.

Good fits: office visits, contact 1:1s, billing/payer calls,
