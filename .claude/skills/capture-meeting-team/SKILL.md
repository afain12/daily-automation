...
- Step 2: Collect meeting notes (paste / file / meetings folder / Granola
  recap page)
- Step 2.5: sensitive info input gate (`scripts/phi_scan.sh`)
...
- Step 5: Present trust gate; the operator selects which items to route
...
happens routinely — the operator edits categorizations and re-displays).
...
If Step 5 approved zero items (the operator picked "None — just save the notes"),
...
>    primary business, Status = "In progress", Assignee = the operator, link
...
>    `id` as `SUBTASK_IDS[]`.
>
> 3. For each the operator-owned subtask: create a Google Task mirror via
...
**Owns Step 6c (Contact CRM upsert).**

> For each approved PROVIDER item:
>
> 1. Search Contact CRM by title contains `<contact name>`
...
- **Worker 1 (notion-tasks) parent creation fails** → subtasks cannot be
  ...
  results still land. the operator re-runs after fixing the parent issue (often
  auth) and re-run safety (Step 6a-3 §re-run) avoids dupes.
- **sensitive info gate fires** in Steps 2.5 → no team is ever created. Refuse and log
  per baseline.
...
4. **Notion writes** target different surfaces. This skill CREATES
   ...
   *while* this skill is creating it under the same parent, but the
   ...
   Mitigation: extremely unlikely in practice (the operator isn't running EOD
...
5. **Trust gates** are interactive — the operator is in one or the other, never
