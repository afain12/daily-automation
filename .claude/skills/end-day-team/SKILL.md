...
`feedback_top3_vs_actual.md` (in-person Product days score 0/3 because work
...
> 2. **Today's braindump text** — the part of the daily note the operator has typed
...
> - `${VAULT}/meetings/**/*.md`
> - `${VAULT}/inbox/**/*.md`
> - business folders `${VAULT}/Product/`, `Operations/`, `Product/`,
>   `Sales/`, `Notes to self/`
>
> Return JSON:
...
> - Also: implicit decisions — "Contact Person F will reschedule to Tuesday"
>   (decision: rescheduling), "Person W needs BAA before referral flow"
>   (decision: gating)
>
> Entity mentions to extract:
> - Contact names (Dr / office / urgent care / "spoke with X")
> - Business names (Strategic Initiative, Example Payer, Example Account, Operations, etc.)
> - Key teammate names (Adam, Valentyna, Person K, Julie, Gary, Person R, Person U,
>   Person W, Person V, Person B, Person S)
>
...
> - 0.60  : implicit decision, requires the operator's call
...
- **Worker 5 (braindump) skipped or empty** → render the "Real Movement
  ...
  detected." Don't treat as failure; some days the operator just doesn't braindump.
...
5. **Trust gates** are interactive (AskUserQuestion). the operator can only be in
