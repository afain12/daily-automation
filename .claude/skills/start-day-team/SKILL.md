...
> You are the calendar worker for Aaron's COO Twin /start-day fan-out. Run
...
> generic terms (lab, lincoln, ipa, nestmate, cardio) AND specific proper
> nouns Aaron uses (LDX, AHBD, Telcor, Gary biller, AFC, Diangelo, Goyal,
> Yili, Rybstein, Sorkin, Roman, Luis, Adrianova, etc.). The lists are
> non-exhaustive — they were expanded 2026-05-19 to fix the validation-run
> issue where real provider visits landed `untagged`.
>
> **Pass 2: fallback inference** for events Pass 1 left `untagged`:
> - Summary starts with `Visit Dr ` or `Visit <Clinic>` → **nestmate**
>   (Aaron's default route for unmatched provider visits)
...
> You are the notion worker for Aaron's COO Twin /start-day fan-out. Query
...
> Master Tasks / Provider CRM / Activity Log, filter `last_edited_time`
...
> `days_since_last_contact > 14` (the `provider_followup_days` value in
> config/sources.yaml). Do NOT include providers contacted 7–14 days ago as
...
>  "master_tasks":{
>    "open_count":29,
>    "in_progress":[{"id":"abc","title":"...","due":null,"workspace":"Nestmate","last_edited_time":"...","last_activity":"gtask:XYZ"}],
>    "not_started":[...],
>    "waiting":[...],
>    "overdue":[...],
>    "stale":[...]
...
> You are the obsidian worker for Aaron's COO Twin /start-day fan-out. Vault
> path: `${VAULT}`. Do this:
>
...
> You are the gtasks worker for Aaron's COO Twin /start-day fan-out. Run via
> Bash:
>
> 1. Open tasks today:
...
> response. Do NOT summarize, sample, or stop after the first few. Aaron
