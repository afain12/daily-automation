...
```json
{
  ...
  "tasks": [
    {
      "title": "High-value contact subset follow-up",
      "workspace": "Operations",
      "subset": "contact-followups",
      "attention_pct": 45,
      "value_score": 90,
      "status": "Not started",
      "due": "2026-06-22",
      "done_when": "contact next step is scheduled",
...
Recommended:
- `workspace`: Operations, Sales, Product, Product, Other.
- `subset`: small high-value bucket, e.g. `contact-followups`, `dispatch-risk`, `sales-replies`, `waiting-on-operator`, `today-overdue`.
...
- `waiting-on-operator`: tasks blocked on the operator response/decision.
- `contact-followups`: Contact CRM / Operations follow-ups likely to move revenue.
