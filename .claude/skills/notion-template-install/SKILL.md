...
description: >-
  ...
  databases (Master Tasks, Provider CRM, Activity Log, Meeting Notes) from
...
master-tasks data source id into the `Related Tasks` relations on Provider CRM
and Meeting Notes.
...
ORDER       = ["master_tasks", "provider_crm", "activity_log", "meeting_notes"]
```
...
| `provider_crm`   | `templates/notion/provider_crm.json` | `master_tasks`   |
| `activity_log`   | `templates/notion/activity_log.json` | —                |
| `meeting_notes`  | `templates/notion/meeting_notes.json`| `master_tasks`   |

**master_tasks MUST be created first** — Provider CRM and Meeting Notes carry
...
selection but `provider_crm` or `meeting_notes` is, abort with a clear error —
...
Which databases should I create? (default: all 4)
  ...
  2. Provider CRM   — contacts / pipeline (depends on Master Tasks)
...
user picked `provider_crm` or `meeting_notes` WITHOUT `master_tasks`, refuse:

```
🛑 Cannot install 'provider_crm' or 'meeting_notes' without 'master_tasks' —
...
About to create N databases under parent page <PARENT_PAGE_ID>:
  ...
  │                Participants (people), Tags (multi_select)               │
  │                                                                         │
  │ 4. Meeting Notes                                                        │
...
2. Running `/notion-probe` immediately afterwards scores each created DB at
   ≥ 0.80 confidence for its target role — Master Tasks ≈ 1.00, Provider CRM
