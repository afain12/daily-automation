...
description: >-
  ...
  Trust gate before any writes; sensitive info gate before any LLM call.
...
You are acting as the operator's Chief of Staff. Your job is to read today's daily-note
...
Read today's daily note and extract the Braindump section. the operator's daily notes
...
for reference):**` placeholder pattern the operator uses in 5/14–5/19 is empty by
...
# TODO(v0.2): T10 — class B Obsidian vault scan (recent edits in Product/,
# Operations/, Product/, Sales/, Notes to self/, notes/, inbox/ within 24h).
...
## Step 2 — sensitive info Input Gate (AAC GATED)

Pipe the braindump through `scripts/phi_scan.sh` BEFORE any LLM call. sensitive info
...
if [ -n "${BRAINDUMP_TEXT// /}" ]; then
  ...
  sensitive info_EXIT=0  # empty input: don't trip phi_scan's exit=2 read-error path.
fi
```

- `sensitive info_EXIT == 0` → clean, proceed to Step 3.
- `sensitive info_EXIT == 1` → sensitive info markers detected. **Refuse.** Display sanitized
...
- `sensitive info_EXIT == 2` → read error (empty input). Treated as no-op upstream.

```bash
if [ "${sensitive info_EXIT}" = "1" ]; then
  TS_sensitive info="$(date -Iseconds 2>/dev/null || date +"%Y-%m-%dT%H:%M:%S%z")"
  export TS_sensitive info RUN_ID
  python - <<'PY' >> "${LOGS_DIR}/_phi_refusals.jsonl"
import json, os
row = {
  "ts": os.environ["TS_sensitive info"],
  "skill": "sync-sweep",
...
PY
  echo "🛑 sensitive info detected in braindump. /sync-sweep refuses. Clean the daily note and re-run."
  STATUS="refused"
fi
```
...
if [ "${STATUS:-ok}" = "refused" ]; then
  : "sensitive info gate refused — skip directly to Step 11 telemetry."
else
...
# Flat lookup: lowercase keyword -> workspace tag (lab/sales/product/product).
kw_to_biz = {}
...
```text
You are extracting entity mentions from the operator's daily-note braindump for the
/sync-sweep skill. the operator tracks contacts, accounts/offices, deals/programs,
...
IN-SCOPE ENTITIES
- People: contacts, reps, business contacts (e.g. "Example Contact", "Dr Gordin", "Person U")
- Accounts / offices: e.g. "Example Account", "Strategic Initiative", "Example Account Healthcare"
- Deals / programs: e.g. "the Example Account product meeting", "Vendor A/Vendor B integration"
- Topic / recurring meetings: e.g. "Product Rollout", "Customer Onboarding weekly"
...
CONFIDENCE RUBRIC
- 0.95+: full name + business context match (e.g., "Example Contact finally called back")
...
- 0.70:  first name + business context, multi-dept name (e.g., "Alex from Example Account product")
...
- If the line contains an SSN/MRN/DOB pattern (sensitive info gate should have caught it,
  but defense in depth): set `entity_canonical: "sensitive info_LEAK"`, confidence 0.0.
...
candidates including topic-page hits (do NOT filter to DB hits — the operator's
...
print(json.dumps({
  ...
  # Example Contact GI scored top for every Product person query.
  #
  ...
  # surface low-similarity candidates in the trust gate — that trains the operator
...
- `parent_db` ("Contact CRM" | "Master Tasks" | "Activity Log" | "Meeting Notes"
...
search calls and surface to the operator at Step 8 with `NOTION_API_TOKEN expired`.
...
- **workspace_match** — 1.0 if candidate's workspace matches the entity's
  ...
  dominating every Product person query at top-recency.
...
e.g., `Person N` → `Product`), apply the override in two strictly-ordered phases:
...
the confidence boost so a name that maps to Product but only returns Lab
...
and Step 9 (in case the trust gate sits open while the operator context-switches).
...
`page_id`, and the `action_id`. If the operator declines at Step 8, the file is
...
and proposed append body. Then ask the operator to approve.
...
1. [class-A conf:0.92] Example Contact  →  [notion:23ba…] Example Contact — Contact CRM (Product)
   Source line 47: "Example Contact finally called back — pushing Example Account product to Friday"
   Append heading: "## 2026-05-19 — Acosta called back, pushing Example Account Friday"
   Append body  : "Example Contact finally called back — pushing Example Account product to Friday"
   match_score  : 0.88 (title 0.95, workspace 1.0, recency 0.70)
   action_id    : sync-sweep:23ba3158:2026-05-19:a1b2c3d4
   url          : https://www.notion.so/...

2. [class-A conf:0.72] Example Contact  →  [notion:8f4d…] Example Contact — Contact CRM (Operations)
   Source line 14: "Example Contact successfully closed for lab"
   E1 note      : line keyword "lab" OVERRODE section "Product" → routing Operations.
   Append heading: "## 2026-05-19 — Example Contact closed for lab"
   match_score  : 0.74
   action_id    : sync-sweep:8f4d2a99:2026-05-19:b2c3d4e5

## Disambiguation needed ({D} items)

3. [class-A conf:0.65] Person U  →  multiple candidates
   Source line 31: "Person U sent over the panel results"
   Pick one:
     [A] Person U Khan — Contact CRM (Product)   last edited 2026-05-12   match: 0.71
     [B] Person U Sayed — Contact CRM (Operations) last edited 2026-04-28 match: 0.62
     [N] None / skip this mention

## Uncategorized ({U} items) — conf < 0.70 OR no match

4. [class-A conf:0.55] "Saw Person H briefly" — no clear entity match
...
- **Edit before applying** — the operator rewrites a specific `append_body` before
  approval. Sets `OPTION_PICKED=D` and re-renders the trust gate.

For disambiguation rows (item 3 in the markdown above), the operator picks `[A]` /
...
if [ "${OPTION_PICKED}" = "C" ]; then
  SKIP_PATCH=1
  echo "ℹ️  the operator declined all staged appends. Archiving payload only."
fi
```
...
# convention (the GROUNDED discipline) so the operator can scan staged appends and
...
the heredoc body). This mirrors `scripts/phi_scan.sh`'s `sensitive info_SCAN_INPUT`
...
```
~~Example Contact finally called back — pushing Example Account product to Friday~~ <!-- synced to notion:23ba3158-... -->
```
...
# (no-match / declined-by-the operator with rationale) into the next day's carries.
...
Emit exactly one row to `logs/_telemetry.jsonl` per run, including sensitive info-refused
...
- `refused` — sensitive info gate refused at Step 2.
...
- **phi_scan.sh exits 1 (sensitive info):** Refuse, log to `_phi_refusals.jsonl`,
...
- **BOUNDED** — Step 0 mode gate; Step 1 `INPUT_CLASS=A` constant explicitly
  ...
  sensitive info hard-refuse class at Step 2; payment-instruction hard-refuse class in
...
- **GATED** — Three gates: Step 0 (mode), Step 2 (sensitive info), Step 8 (trust gate).
