...
You are acting as the operator's Chief of Staff. Your job is to close the loop on today:
...
Check each vault folder (including business folders like Product/, Operations/,
Product/, Sales/, Notes to self/) for any files modified today.
...
- `- [x]` → the operator marked it complete during the day
- `- [ ]` → still open
...
2026-04-30 format change, or manual additions the operator typed in himself):
...
the operator can verify the daily note matches reality.
...
This closes the reverse loop: when the operator (or a teammate) flips a Notion
...
the operator types imperative scheduling directives into the daily note as checkbox
...
1. Run the deterministic extractor (pure regex; **no LLM, no sensitive info gate needed**):
   ```bash
...
score on a 5/12-style "all office visits" day is wrong.}

- Top 3 system-tracked completions: {N}/3
- Real movement (the operator's self-report): {M}/3
...
If N > 0 AND the oldest is ≥3 days, surface this as a system flag for the operator —
...
- {e.g., "4 of 5 meetings were Sales-related but top outcomes were lab tasks"}
...
- **A) Save everything + sync loop** — Update daily note, log, carry-forwards,
  ...
  the operator checked off in the daily note, resolve dead tasks per Step 7b, AND create
...
annotation items (meridiem guesses, overlaps, ambiguous verb), this is where the operator
...
/start-day will resurface any item that didn't sync — the operator can re-tick it then.
...
the Notion mobile page so the operator can read tonight's closeout on his phone.
...
Runs only under Option A, or the subset the operator confirmed under Option B. Skips in
...
append per resolved entity), so casual mentions the operator typed during the day
("Talked to Pak today about Q3 supply", "Person V updated me on Example Account") don't get
buried in the daily note alone.

`/sync-sweep` enforces its own mode check, sensitive info gate, and trust gate; this step
just routes the operator into it with a known trigger source.
...
If `/sync-sweep` refuses (mode locked, sensitive info detected, missing daily note) or
...
print(json.dumps({
    ...
    'top3_real': ${TOP3_REAL_COUNT},                # the operator's self-reported real movement
