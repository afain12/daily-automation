...
description: >-
  ...
  roles (Master Tasks, Provider CRM, Activity Log, Meeting Notes). Writes a
...
re-derive Aaron's `config/sources.yaml` roles from schema alone, they'll
...
The scorer applies four rubrics — Master Tasks, Provider CRM, Activity Log,
...
**Rubric calibration.** If a role drops below 0.80 against Aaron's
...
When run against Aaron's workspace, the report must identify these four
