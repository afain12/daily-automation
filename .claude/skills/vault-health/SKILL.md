...
description: >-
  ...
  fix-or-remove broken wikilinks). Never auto-fixes — the operator decides.
coo_twin:
  category: admin
  mode_required: any
...
not rule-based — auto-approving them would mean deleting the operator's notes).
...
Cap lists at 20 visible entries with an "... and N more" line. the operator triages
the visible ones; the rest accumulate in JSON for follow-up.

## Step 3: Triage Trust Gate

For each category with issues, use `AskUserQuestion` to ask the operator how to
...
>   in future briefings.
> - **List the first 10 verbosely** — show fuller previews so the operator can pick
>   per-item.
...
>   >2.0), propose renaming the wikilink to the real file. the operator approves
>   per-link.
...
>   filename + body keywords; the operator approves per-note.
