...
description: >-
  ...
  with an Obsidian link. Contact updates go to CRM. Trust gate before any writes.
coo_twin:
  category: capture
  mode_required: any
...
You are acting as the operator's Chief of Staff. Your job is to take raw meeting notes,
...
## Step 2.5: sensitive info Input Gate (AAC GATED — input gate)

Before any LLM parsing, scan the raw notes for sensitive info markers. This is the input gate
required by AAC §2.3 (GATED). It does NOT make the system HSalesA-compliant; it
...
- `sensitive info_EXIT == 0` → clean, proceed.
- `sensitive info_EXIT == 1` → sensitive info markers detected. **Refuse to parse.** Display the
  line numbers + sanitized snippets from phi_scan.sh stderr. Ask the operator to
...
- `sensitive info_EXIT == 2` → read error (empty input). Refuse and ask for actual notes.

Log every sensitive info refusal in `logs/_phi_refusals.jsonl` (append-only) with timestamp,
...
Options:
- **Lab (Operations)** — lab business meeting
- **Sales** — Sales business meeting
- **Product** — Product meeting
- **Product** — Product or Product meeting
...
**Contact Updates** → Notion Contact CRM
- Indicators: "spoke with Dr", "contact said", "practice update",
  "credentialing", "panel size", "identifier"
- Extract: contact name, update details, next step
...
- **Confidence threshold rule:** any item with confidence < 0.70 routes to the
  **Uncategorized** bucket regardless of which indicators matched. the operator classifies
...
- **Carry source citation on every item:**
  ```
  {
    ...
    "confidence": 0.91,
    "source_line": 14,
    "source_text": "Tell Person E to follow up with Contact Person C by Friday",
    "owner": "Person E",  "owner_resolved_from": "attendee match",
...
- When an owner is mentioned, extract it. If no owner is mentioned, default to the operator.
- Parse date references into ISO dates: "next Tuesday" → actual date,
  "by end of week" → that Friday's date, "ASAP" → today. Always record
  `due_resolved_from` so the operator can spot misparses at the trust gate.
...
## Notion: Contact CRM ({count} items)
4. [PROVIDER conf:0.85] Update {contact name} — Last Contact: {date}, Next Step: {detail}
   Source line {N}: "{source_text}"
   action_id: {…}
...
## Uncategorized — needs the operator's call ({count} items)
...
the operator approve in seconds for high-confidence items and read carefully only on the
borderline ones.
...
a single parent task, so the operator sees `[Meeting] Product/Product 2026-04-30` in
...
meeting's primary business tag (the operator will see it in the right department
...
curl -s -X POST "https://api.notion.com/v1/pages" \
  ...
  -d '{
    ...
    "properties": {
      ...
      "Assignee": {"people": [{"id": "<operator_user_id>"}]},
...
curl -s -X POST "https://api.notion.com/v1/pages" \
  ...
  -d '{
    ...
    "properties": {
      ...
      "Assignee": {"people": [{"id": "<resolved attendee or operator>"}]},
...
**Step 6a-3: Google Tasks mirror for the operator-owned items (primary checklist).**

Google Tasks is the operator's primary daily checklist; Notion is the team-visible
archive. Routing rule per subtask:

| Owner | Google Tasks | Notion subtask |
|---|---|---|
| the operator | ✅ create (primary) | ✅ create (audit trail) |
| Teammate (Person U, Guerrechon, etc.) | ❌ skip | ✅ create (it's on their plate) |
| External / track-only (e.g. Person D → Ella) | ❌ skip | ✅ create (the operator is tracking, not doing) |

For each the operator-owned subtask, after the Notion subtask is created, immediately
...
mirror in the routing summary so the operator knows to re-run later.
...
- **Zero action items extracted** → still create the parent task with Status
  ...
  to drop a meeting entirely; the operator can re-run later if action items emerge.
- **Mixed-business action items** (e.g. a Product/Product joint meeting) →
  parent gets the more specific business (Product beats Other). Each
...
### 6c: Update Contact CRM

For each approved contact update, first search for the existing contact:
...
curl -s -X POST "https://api.notion.com/v1/pages" \
  ...
  -d '{
    ...
    "properties": {
      "Task": {"title": [{"text": {"content": "<contact name>"}}]},
      "Last Contact": {"date": {"start": "<TODAY>"}},
      "Next Step": {"rich_text": [{"text": {"content": "<next step>"}}]},
...
If neither works, output the calendar items as text for the operator to add manually.
...
**Updated in Contact CRM:** {count} contacts
...
print(json.dumps({
    ...
    'phi_refused': ${sensitive info_REFUSED_BOOL},
    'writes': {
...
`STATUS` is `ok` (all approved items written), `partial` (≥1 write failed), `refused` (sensitive info gate), or `failed` (Notion auth, total outage).

## Error Handling

- **scripts/check_mode.sh exits 1 (locked):** Refuse. No telemetry — locked runs don't happen.
- **scripts/phi_scan.sh exits 1 (sensitive info detected):** Refuse parsing. Append to `logs/_phi_refusals.jsonl`. Telemetry status = `refused`.
- **Notion API failure:** Log the failed item to `${VAULT}/meetings/{TODAY}-pending-notion.md`
  with all the structured fields so the operator can manually enter it. Continue with other items.
