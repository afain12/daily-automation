...
| Rename a **teamspace** (e.g. `Product` → `Product`) | ❌ | ✅ |
...
2. **Browser DOM (fallback + verification).** Use when the private API path is
   ...
   - **`connect-chrome`** (visible) for **every irreversible step** — the operator watches
...
1. Confirm a logged-in Notion session. Preferred: `connect-chrome` (uses the operator's real
...
2. Navigate to a known page (Command Center `335a3158-59b4-815b-863d-c585bf957dc8`).
   **Screenshot.** If it shows a login wall, STOP — ask the operator to log in (`! ` command)
...
4. **TRUST GATE** — show the operator: the op, the target (id + human name), the mechanism
...
## Current backlog (clean-up-first scope — the operator 2026-05-29)

Do the low-risk wins now; defer hub-page / Company OS Home structure until the cleaned
baseline settles.

1. **Fix or delete the broken Master Tasks "Operations" view** (`view://335a3158-59b4-8116-af5a-000cc458de8c`)
   — its filter still references the deleted `Link & Reference Laboratory` option, so
   it returns nothing. Either repoint the filter to `Operations` or
   delete it (redundant with Workspace-grouped Morning Report).
2. **Delete redundant Master Tasks views**: `Sales`, `Product`, `Product`,
   `Top-level tasks` — replaced by the Workspace-grouped Morning Report + per-hub
   linked views.
3. **Delete `Sales Contacts` view** (Contact CRM) and **`My Notes` view** (Meeting Notes).

Deferred (design-after): rename `Product` → `Product`, reparent the 4 DBs under HQ,
...
- Never type credentials; if logged out, hand back to the operator.
...
- HSalesA-adjacent: never screenshot or download pages containing client identifiers
